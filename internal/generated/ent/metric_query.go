...
func (mq *MetricQuery) QueryOwner() *UserQuery {
...
func (mq *MetricQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (mq *MetricQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (mq *MetricQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (mq *MetricQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (mq *MetricQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (mq *MetricQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (mq *MetricQuery) WithOwner(opts ...func(*UserQuery)) *MetricQuery {
	query := (&UserClient{config: mq.config}).Query()
...
//
//	client.Metric.Query().
//		GroupBy(metric.FieldMetricType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (mq *MetricQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Metric, error) {
	...
	if query := mq.withOwner; query != nil {
		if err := mq.loadOwner(ctx, query, nodes, nil,
			func(n *Metric, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MetricQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Metric, init func(*Metric), assign func(*Metric, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Metric)
	for i := range nodes {
		if nodes[i].user_metrics == nil {
			continue
		}
		fk := *nodes[i].user_metrics
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_metrics" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MetricQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(metric.Table, metric.Columns, sqlgraph.NewFieldSpec(metric.FieldID, field.TypeUUID))
	_spec.From = mq.sql
